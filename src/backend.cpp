...
QUrl Backend::lastOpenDirectory() const {
    const QString path = QSettings().value(lastOpenDirectorySetting,
...
QString Backend::fileName() const {
    ...
    return name.isEmpty() ? QStringLiteral("Untitled.txt") : name;
}
...
void Backend::attachDocument(QObject *textDocument) {
    ...
    if (!quickDocument || !quickDocument->textDocument()) {
        setStatus(QStringLiteral("Could not attach the writing surface."));
        return;
    }
...
void Backend::open(const QUrl &url) {
    ...
    setFileUrl(url);
    watchCurrentFile();
    QSettings().setValue(lastOpenDirectorySetting,
                         QFileInfo(url.toLocalFile()).absolutePath());
    emit lastOpenDirectoryChanged();
    setModified(false);
...
void Backend::printDocument() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        rendered.setPlainText(currentDocumentText());
